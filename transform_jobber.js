...
const title = titleMatch ? titleMatch[1] : 'Jobber CRM Services & Setup | Sage Kite';
...
const canonical = canonicalMatch ? canonicalMatch[1] : 'https://www.sagekite.com/platforms/jobber/';
...
// Clean up styles to prevent bleed:
// remove .site-header, .header-inner, .logo, .nav, .menu-toggle, @media for header
styles = styles.replace(/\/\* Header \*\/[\s\S]*?\/\* 01 Hero \*\//g, '/* 01 Hero */');
// remove .site-footer, .foot-grid, .legal etc
styles = styles.replace(/\/\* Footer \*\/[\s\S]*?\/\* Motion:/g, '/* Motion:');
...
// Handle React camelCase SVG properties
...
// Inject JSON-LD directly to bypass Next.js metadata escaping issues
...
fs.mkdirSync('app/platforms/jobber', { recursive: true });
fs.writeFileSync('app/platforms/jobber/page.tsx', output);
console.log('Successfully created app/platforms/jobber/page.tsx');
